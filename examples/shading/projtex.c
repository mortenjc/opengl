...
float xmin = -0.035f, xmax = 0.035f;
float ymin = -0.035f, ymax = 0.035f;
float znear = 0.1f;
float zfar = 1.9f;
float distance = -1.0f;
...
initialize(void)
{
  ...
  /* Setup Misc.  */
  glClearColor(0.41f, 0.41f, 0.31f, 0.0f);
...
display(void)
{
  ...
  if (textureEnabled) {
    ...
    if (showProjection) {
      ...
      drawTextureProjection(); //draws wires showing projection
      glEnable(GL_LIGHTING);
      glPopMatrix();
    }
...
//    glEnable(GL_TEXTURE_GEN_Q); /* not needed -tomcat */
  }
...
//  glDisable(GL_TEXTURE_GEN_Q); /* not needed -tomcat */
...
create_menu(void) {
    glutCreateMenu(menu);
    glutAddMenuEntry("change object", 'o');
    glutAddMenuEntry("toggle frustum display", 's');
    glutAddMenuEntry("toggle projected texture", 't');
    glutAddMenuEntry("toggle linear/nearest filter", 'l');
